...
void MPU6050_Init(){	
	...
	// wiringPiI2CWriteReg8 (fd, ACCEL_CONFIG, 0x10);	// Write to Accel
	// wiringPiI2CWriteReg8 (fd, GYRO_CONFIG, 0x10);	// Write to Gyro Configuration register
...
int main(){
    ...
    GyroX = GyroX + 0.069; // GyroErrorX ~(-0.56)
    GyroY = GyroY + 0.081; // GyroErrorY ~(2)
    GyroZ = GyroZ + 0.093; // GyroErrorZ ~ (-0.8)
